...
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/chat_system?...&allowPublicKeyRetrieval=true
    username: root
...
> 需要本地安装 `mysql` 客户端用于迁移。如果没有，手动登录 MySQL 执行 `db/migration/` 下的 SQL 文件后跳过迁移步骤。
...
迁移脚本按文件名排序依次执行所有 SQL（启动脚本已自动调用，一般无需手动执行）：
```bash
...
| B. 私聊与实时通信 | B | ✅ 完成 | WebSocket/私聊消息/已读状态/离线消息 |
| C. 群聊 | C | ✅ 完成 | 群 CRUD/群成员管理/群消息/群公告 |
| D. 好友与分组 | D | ✅ 完成 | 好友申请/分组管理/黑名单/好友搜索 |
| E. 消息中心与文件 | E | ✅ 完成 | 聊天记录查询/导出/图片文件消息/表情 |
| F. 语音与管理后台 | F | ✅ 完成 | 语音消息/管理员功能/敏感词过滤 |
